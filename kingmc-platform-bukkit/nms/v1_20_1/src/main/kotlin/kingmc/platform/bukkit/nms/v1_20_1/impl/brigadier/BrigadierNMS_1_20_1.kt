...
import kingmc.platform.bukkit.nms.v1_20_1.NMS_1_20_1
...
@Suppress("ClassName")
@Component("brigadierNMS_1_20_1")
@ConditionalOnVersion("1.20.1")
@ConditionalOnBean([BrigadierNMS::class])
class BrigadierNMS_1_20_1 @Autowired constructor(
    val audienceFactory: AudienceFactory,
    val nms: NMS_1_20_1,
    val commandContextFactory: CommandContextFactory,
    val commandMap: CommandMap
) : BrigadierNMS<CommandListenerWrapper> {

    init {
        withApplication {
            info("Using ${this@BrigadierNMS_1_20_1::class.qualifiedName} as brigadier support...")
        }
    }
    ...
    private val sendAsync: Method =
        getCommandDispatcher().javaClass.getDeclaredMethod("sendAsync", NMSEntityPlayer_1_20_1::class.java)
    ...
    private fun sendCommandsAsync(entityplayer: EntityPlayer) {
        ...
        if (SpigotConfig.tabComplete >= 0) {
            ...
            val vanilla = entityplayer.d.vanillaCommandDispatcher.a().root
            map[vanilla] = vanillaRoot
            this.a(vanilla, vanillaRoot, entityplayer.da(), map)
            ...
            entityplayer.c.a(PacketPlayOutCommands(rootcommandnode))
        }
    ...
    private fun deserializeCommandHandlerParameters(
        ...
        return deserializeCommandParameter(deserializing, this@BrigadierNMS_1_20_1.application).apply {
                    executes { css ->
                        return@executes withApplication(handler.application) {
                            try {
                                val parameters = BrigadierParameters_1_20_1(css, listed.subList(0, index + 1))
                                val commandContext = commandContextFactory.create(getCommandSender(css), parameters, css.input)
                                handler.invoke(commandContext).asInt()
                            } catch (e: Exception) {
                                printCommandHandleException(e)
            ...
            .apply {
                ...
                if (suggestion != null) {
                    if (suggestion is BlockingSuggestionProvider) {
                        suggests(BlockingSuggestionProvider_1_20_1(suggestion, this@BrigadierNMS_1_20_1, application = application))
                    }
                    if (suggestion is SuspendSuggestionProvider) {
                        suggests(SuspendSuggestionProvider_1_20_1(suggestion, this@BrigadierNMS_1_20_1, application = application))
                    }
                }
            }
    }
